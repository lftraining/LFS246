...
@Configuration
public class CustomerChannelSubscription {
	
	private static Logger LOGGER = LoggerFactory.getLogger(CustomerChannelSubscription.class);
	...
	@Bean
	public Consumer<Message<Object>> blockAmountCS(){
		return this::doBlockAmountCS;
	}
	
	private void doBlockAmountCS(Message<Object> msg) {
		...
		if(orderDetails != null && orderDetails.getCustId() != null) {
			LOGGER.info("customer-service-es :: Order Details CS :: " + orderDetails);
			LOGGER.info("customer-service-es :: Order Status CS :: " +  orderDetails.getOrderStatus());
	        ...
	        if (CustomerOrderStatus.NEW.toString().equalsIgnoreCase(orderDetails.getOrderStatus())) {
	            ...
	            try {
					kafkaTemplate.send(TOPIC_NAME_CS, this.generateCloudEvent(orderDetails));
				} catch (JsonProcessingException e) {
	        ...
	        LOGGER.info("Customer-service-es :: Order details CS :: " + customerDetails);
        }
	}
